...
function keyStr2Bytes(key){
    const list=key.split('\n')
    const str = list.slice(1,list.length-1).join('')
...
props.initKeys = async function(){
    this.rsaPrivate = await crypto.subtle.importKey(
        'pkcs8',
        keyStr2Bytes(this.rsaPrivateRaw),
        {
    ...
    this.rsaPublic = await crypto.subtle.importKey(
        'spki',
        keyStr2Bytes(this.rsaPublicRaw),
        {
            name: "RSASSA-PKCS1-v1_5",
            hash: "SHA-256"
        },
 ...
 */
props.appPay = async function (opts) {
    ...
    return await utl.processParams(params, this.rsaPrivate);
};
 ...
 */
props.wapPay = async function (opts) {
 ...
 * @param {String} [opts.return_url]         客户端回调地址，HTTP/HTTPS开头字符串
 */
props.pagePay = async function (opts) {
 ...
 * @param {String} [opts.disablePayChannels] 禁用渠道，用户不可用指定渠道支付。当有多个渠道时用“,”分隔。 注：与enable_pay_channels互斥
 */
props.preCreate = async function (opts) {
    var biz_content = {
        out_trade_no: opts.outTradeId,
        subject: opts.subject,
        total_amount: opts.amount,
 ...
 * @param {String} [opts.appAuthToken]  https://doc.open.alipay.com/doc2/detail.htm?treeId=216&articleId=105193&docType=1
 */
props.close = async function (opts) {
 ...
 * @param {String} [opts.appAuthToken]  https://doc.open.alipay.com/doc2/detail.htm?treeId=216&articleId=105193&docType=1
 */
props.refundQuery = async function (opts) {
 ...
 * @param {String} [opts.appAuthToken] https://doc.open.alipay.com/doc2/detail.htm?treeId=216&articleId=105193&docType=1
 */
props.billDownloadUrlQuery = async function (opts) {
