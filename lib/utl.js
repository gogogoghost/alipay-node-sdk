...
utl.signVerify = async function (str, sign, publicKey) {
    ...
    var verify = await crypto.subtle.verify('RSASSA-PKCS1-v1_5',publicKey,signBytes.buffer,encoder.encode(str))
    return verify;
};
...
utl.sign = async function (str, privateKey) {
    const resBytes = await crypto.subtle.sign('RSASSA-PKCS1-v1_5',privateKey,encoder.encode(str).buffer)
